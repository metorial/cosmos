#!/bin/bash
# Script to add Vault configuration to cosmos agents on all nodes
...
echo "Adding Vault configuration to cosmos agents on all nodes..."
...
for NODE in "${NODES[@]}"; do
  ...
  ssh -o StrictHostKeyChecking=no -J "$BASTION" "ubuntu@$NODE" << 'ENDSSH'
    ...
    # Update cosmos-agent service with Vault config
...
EOF
    ...
    echo "Node fixed and cosmos-agent restarted"
ENDSSH
...
echo "All nodes have been updated with Vault configuration!"
